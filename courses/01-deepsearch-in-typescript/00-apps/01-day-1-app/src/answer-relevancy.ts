...
function generateEvaluationStatementsPrompt({
  ...
  return `${ANSWER_RELEVANCY_AGENT_INSTRUCTIONS}
...
export async function checkAnswerRelevancy(opts: {
  ...
  // First, break down the submission into statements
  const { object: statementsObj } = await generateObject({
  ...
  // If no statements, return a low score
  if (statementsObj.statements.length === 0) {
    return {
      score: 0,
      metadata: {
        statements: [],
  ...
  const { object: evaluationObj } = await generateObject({
    ...
    prompt: generateEvaluatePrompt({
      input: opts.question,
      statements: statementsObj.statements,
    }),
    schema: z.object({
      verdicts: z.array(
        z.object({
  ...
  // Convert verdicts to scores and average them
  const scores = {
    yes: 1,
    no: 0,
    unsure: 0.5,
  ...
  const totalScore = evaluationObj.verdicts.reduce(
    (acc, verdict) => acc + scores[verdict.verdict],
    0
  );
  const averageScore = totalScore / evaluationObj.verdicts.length;

  return {
    score: averageScore,
    metadata: {
      statements: statementsObj.statements,
      verdicts: evaluationObj.verdicts,
    },
  };
} 